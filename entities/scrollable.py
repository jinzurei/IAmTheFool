...
class ScrollableEntity:
    ...
    def update(self, dx: float) -> None:
        ...
        Positive dx moves content to the right (camera moving left), negative dx moves left.
        """
        self.x1 += dx
        self.x2 += dx
        # If a segment moves completely off to the left, wrap it to the right of the other
